...
class FTPClient:
    ...
    def create_connection_frame(self):
        ...
        self.server_entry.insert(0, "ftp.dlptest.com")  # Default for testing
        ...
        self.user_entry.insert(0, "dlpuser")  # Default for testing
        ...
        self.pass_entry.insert(0,"rNrKYTX9g7z3RgJRmxWuGHbeu") # Default for Testing for convinence 
    ...
    def _connect_thread(self, server, port, username, password):
        ...
        try:
            self.ftp = FTP()
            self.ftp.connect(server, port, timeout=30)
    ...
    def populate_remote_tree(self, path=None):
        ...
        try:
            ...
            for item in items:
                # Parse FTP directory listing (simple parser)
                parts = item.split()
                if len(parts) < 9:
                    continue
    ...
    def upload_file(self):
        ...
        if not self.connected:
            messagebox.showwarning("Upload", "Not connected to FTP server")
            return
        ...
        remote_path = os.path.join(remote_dir, filename).replace(os.sep, '/')
        
        print(f"Uploading from {local_path} to {remote_path}")  # Debug log
        ...
        threading.Thread(target=self._upload_thread, args=(local_path, remote_path, remote_dir), daemon=True).start()
    
    def _upload_thread(self, local_path, remote_path, remote_dir):
        ...
        try:
            ...
            with open(local_path, "rb") as f:
                self.ftp.storbinary(f"STOR {os.path.basename(remote_path)}", f)
            
            self.root.after(0, self._upload_complete, remote_path)
        except error_perm as e:
            self.root.after(0, self._upload_failed, f"Permission denied: {str(e)}")
        except error_temp as e:
            self.root.after(0, self._upload_failed, f"Temporary error: {str(e)}")
        except Exception as e:
            self.root.after(0, self._upload_failed, f"Unexpected error: {str(e)}")
    ...
    def create_remote_folder(self):
        ...
        remote_path = os.path.join(self.current_dir, folder_name).replace(os.sep, '/')
